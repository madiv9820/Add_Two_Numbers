...
def _create_linked_list(values: List[int]) -> Optional[ListNode]:
    # 🔗 Build a linked list from the given values and create a cycle if pos >= 0.
    if not values:
        return None
...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        l1:                 List[int]   = testcase['input']['l1']
        l2:                 List[int]   = testcase['input']['l2']
        ...
        try:
            l1              : Optional[ListNode]    = _create_linked_list(values=l1)
            l2              : Optional[ListNode]    = _create_linked_list(values=l2)
            head            : Optional[ListNode]    = self._run_case(l1=l1, l2=l2)
            actual_output   : List[int]             = []

            while head:
                actual_output.append(head.val)
                head = head.next
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        return self.__solution.addTwoNumbers(l1=l1, l2=l2)

class AddTwoNumbersTestResult(unittest.TextTestResult):
    ...
    def stopTestRun(self):
        ...
        print('\n' + '🧾' + '=' * 58)
        print('📚 AddTwoNumbers Final Summary')
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=AddTwoNumbersTestResult)
    )
